...
#include "ghost1_png.bin.h"
#include "ghost2_png.bin.h"
#include "ghost3_png.bin.h"
#include "ghost4_png.bin.h"
#include "ghost5_png.bin.h"
#include "ghost6_png.bin.h"
#include "ghost7_png.bin.h"
#include "ghost8_png.bin.h"


PngDatas texture_ghost  [8]; // PNG container of texture
...
void Load_PNG()
{
    ...
    texture_ghost[0].png_in   = (void *) ghost1_png_bin;
    texture_ghost[0].png_size = sizeof  (ghost1_png_bin);

    texture_ghost[1].png_in   = (void *) ghost2_png_bin;
    texture_ghost[1].png_size = sizeof  (ghost2_png_bin);

    texture_ghost[2].png_in   = (void *) ghost3_png_bin;
    texture_ghost[2].png_size = sizeof  (ghost3_png_bin);

    texture_ghost[3].png_in   = (void *) ghost4_png_bin;
    texture_ghost[3].png_size = sizeof  (ghost4_png_bin);

    texture_ghost[4].png_in   = (void *) ghost5_png_bin;
    texture_ghost[4].png_size = sizeof  (ghost5_png_bin);

    texture_ghost[5].png_in   = (void *) ghost6_png_bin;
    texture_ghost[5].png_size = sizeof  (ghost6_png_bin);

    texture_ghost[6].png_in   = (void *) ghost7_png_bin;
    texture_ghost[6].png_size = sizeof  (ghost7_png_bin);

    texture_ghost[7].png_in   = (void *) ghost8_png_bin;
    texture_ghost[7].png_size = sizeof  (ghost8_png_bin);
    
    // load PNG from memory

    for(i = 0; i < 8; i++)
        LoadPNG(&texture_ghost[i], NULL);
